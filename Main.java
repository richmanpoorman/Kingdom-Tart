...
public class Main extends JPanel implements Runnable{
	...
	ArrayList<Enemy> enemiesInBattle = new ArrayList<Enemy>(); 
	...
	public Main() {
		...
		Enemy e = new Enemy("John Adams", 100, 100, 100, 100,100,100,"");
	...
	public void setRecipesOpen() { // WILL FIX
		int i = 0;
		for(int o = 0; o < 4; o++) {
			if(battleParty[i] != null) {
				for (String name : battleParty[i].ownRecipes) {
	...
	public Character setEnemy (boolean isTeam) {
		int target = 0;
		if(isTeam) {
			for(Enemy e : enemiesInBattle) {
				if(e.hp>0) {
					break;
				}
				target++;
			...
			for(MainCharacter m : battleParty) {
				if(m.hp>0) {
					break;
				}
				target++;
		...
		if(isTeam)return enemiesInBattle.get(target);
		return battleParty[target];
